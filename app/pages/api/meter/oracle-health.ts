  ...
  try {
    ...
    const providerDetails = providers.map(provider => ({
      ...
      uptime: provider.lastSuccess
        ? (Date.now() - (provider.lastFailure || provider.lastSuccess)) /
          (1000 * 60 * 60) // hours
        : 0,
      status:
        provider.enabled && provider.consecutiveFailures === 0
          ? 'healthy'
          : provider.enabled && provider.consecutiveFailures < 3
            ? 'degraded'
            : provider.enabled
              ? 'unhealthy'
              : 'disabled',
    ...
    return res.status(200).json({
      ...
      consensusConfig,
      overallStatus:
        healthStatus.healthyProviders >= consensusConfig.minSources
          ? 'healthy'
          : 'degraded',
    });
  } catch (error) {
