      ...
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4'>
        ...
        <div className='bg-green-50 p-4 rounded-lg'>
          ...
          <p className='text-2xl font-bold text-green-900'>
            {formatNumber(data.activeProjects)}
          </p>
        </div>
        <div className='bg-yellow-50 p-4 rounded-lg'>
          ...
          <p className='text-2xl font-bold text-yellow-900'>
            {formatNumber(data.fundedProjects)}
          </p>
        </div>
          ...
          <div className='h-64 flex items-end space-x-2'>
            {data.fundingTrends.map((trend, index) => {
              const maxAmount = Math.max(
                ...data.fundingTrends.map(t => t.amount)
              );
              const height =
      ...
      <div className='grid grid-cols-1 lg:grid-cols-2 gap-6'>
        <div>
          ...
          <div className='space-y-3'>
            {Object.entries(data.fundingByCategory).map(
              ([category, amount]) => {
                const total = Object.values(data.fundingByCategory).reduce(
                  (sum, val) => sum + val,
                  0
                );
        ...
        <div>
          ...
          <div className='space-y-3'>
            {Object.entries(data.projectsByStatus).map(([status, count]) => {
              const total = Object.values(data.projectsByStatus).reduce(
                (sum, val) => sum + val,
                0
              );
