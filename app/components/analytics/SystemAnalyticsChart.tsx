...
export const SystemAnalyticsChart: React.FC<SystemAnalyticsChartProps> = ({
  data,
}) => {
    ...
    <div className='space-y-8'>
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4'>
        ...
        <div className='bg-purple-50 p-4 rounded-lg'>
          ...
          <p className='text-2xl font-bold text-purple-900'>
            {formatNumber(data.apiRequests.total)}
          </p>
        </div>
      </div>
          ...
          <div className='grid grid-cols-1 md:grid-cols-3 gap-6'>
            <div className='text-center'>
              ...
              <div className='text-xs text-gray-500 mt-1'>
                {(
                  (data.memoryUsage.heapUsed / data.memoryUsage.heapTotal) *
                  100
                ).toFixed(1)}
                % of heap
              </div>
      ...
      <div className='grid grid-cols-1 lg:grid-cols-2 gap-6'>
        <div>
          ...
          <div className='bg-white border border-gray-200 rounded-lg p-4'>
            <div className='space-y-3'>
              {Object.entries(data.apiRequests.byEndpoint).map(
                ([endpoint, count]) => {
        ...
        <div>
          ...
          <div className='bg-white border border-gray-200 rounded-lg p-4'>
            <div className='space-y-3'>
              {Object.entries(data.apiRequests.byMethod).map(
                ([method, count]) => {
                  const total = data.apiRequests.total;
                  const percentage = total > 0 ? (count / total) * 100 : 0;
        ...
        <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4'>
          <div className='bg-white border border-gray-200 rounded-lg p-4'>
            <div className='flex items-center justify-between mb-2'>
              ...
              <div
                className={`w-3 h-3 rounded-full ${data.databaseConnections > 0 ? 'bg-green-500' : 'bg-red-500'}`}
              ></div>
            </div>
          ...
          <div className='bg-white border border-gray-200 rounded-lg p-4'>
            <div className='flex items-center justify-between mb-2'>
              ...
              <div
                className={`w-3 h-3 rounded-full ${data.errorRate < 5 ? 'bg-green-500' : data.errorRate < 10 ? 'bg-yellow-500' : 'bg-red-500'}`}
              ></div>
            </div>
            <div className='text-2xl font-bold text-gray-900'>
              {data.errorRate.toFixed(2)}%
          ...
          <div className='bg-white border border-gray-200 rounded-lg p-4'>
            <div className='flex items-center justify-between mb-2'>
              ...
              <div
                className={`w-3 h-3 rounded-full ${data.responseTime.average < 500 ? 'bg-green-500' : data.responseTime.average < 1000 ? 'bg-yellow-500' : 'bg-red-500'}`}
              ></div>
            </div>
            <div className='text-2xl font-bold text-gray-900'>
              {data.responseTime.average.toFixed(0)}ms
          ...
          <div className='bg-white border border-gray-200 rounded-lg p-4'>
            <div className='flex items-center justify-between mb-2'>
              ...
              <div
                className={`w-3 h-3 rounded-full ${(data.memoryUsage.heapUsed / data.memoryUsage.heapTotal) * 100 < 80 ? 'bg-green-500' : (data.memoryUsage.heapUsed / data.memoryUsage.heapTotal) * 100 < 90 ? 'bg-yellow-500' : 'bg-red-500'}`}
              ></div>
            </div>
            <div className='text-2xl font-bold text-gray-900'>
              {(
