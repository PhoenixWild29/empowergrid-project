      ...
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4'>
        ...
        <div className='bg-yellow-50 p-4 rounded-lg'>
          ...
          <p className='text-2xl font-bold text-yellow-900'>
            {formatNumber(data.verifiedUsers)}
          </p>
        </div>
          ...
          <div className='h-64 flex items-end space-x-2'>
            {data.userGrowth.slice(-30).map((growth, index) => {
              const maxCumulative = Math.max(
                ...data.userGrowth.map(g => g.cumulative)
              );
              const height =
                maxCumulative > 0
                  ? (growth.cumulative / maxCumulative) * 100
                  : 0;
              ...
              return (
                <div key={index} className='flex-1 flex flex-col items-center'>
                  ...
                  <div className='text-xs text-gray-500 mt-2 transform -rotate-45 origin-top'>
                    {new Date(growth.date).toLocaleDateString('en-US', {
                      month: 'short',
                      day: 'numeric',
                    })}
                  </div>
        ...
        <div className='bg-white border border-gray-200 rounded-lg p-4'>
          <div className='space-y-4'>
            {data.reputationDistribution.map((range, index) => {
              const total = data.reputationDistribution.reduce(
                (sum, r) => sum + r.count,
                0
              );
      ...
      <div className='grid grid-cols-1 md:grid-cols-3 gap-4'>
        <div className='bg-indigo-50 p-4 rounded-lg'>
          ...
          <p className='text-xl font-bold text-indigo-900'>
            {formatNumber(data.newUsersThisWeek)}
          </p>
        </div>
