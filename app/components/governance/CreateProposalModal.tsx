    ...
    try {
      ...
      switch (formData.type) {
        case ProposalType.PROJECT_FUNDING:
          if (!formData.projectId || !formData.fundingAmount || formData.fundingAmount <= 0) {
            throw new Error('Project funding requires valid project ID and funding amount');
          }
          break;
        case ProposalType.MILESTONE_APPROVAL:
          if (!formData.projectId || !formData.milestoneId) {
            throw new Error('Milestone approval requires project ID and milestone ID');
          }
          break;
        case ProposalType.PARAMETER_CHANGE:
          if (!formData.targetContract || !formData.targetFunction) {
            throw new Error('Parameter change requires target contract and function');
          }
          break;
        case ProposalType.TREASURY_ALLOCATION:
          if (!formData.fundingAmount || formData.fundingAmount <= 0) {
            throw new Error('Treasury allocation requires valid funding amount');
          }
          break;
      }
      ...
      logger.info('Proposal created successfully', { proposalId: result.data.id });
      ...
      logger.error('Failed to create proposal', { error: (err as Error).message });
  ...
  const renderTypeSpecificFields = () => {
    switch (formData.type) {
      case ProposalType.PROJECT_FUNDING:
        return (
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium mb-1">Funding Amount (SOL)</label>
              <input
                type="number"
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setFormData({ ...formData, targetFunction: e.target.value })}
                className="w-full px-3 py-2 border border-gray-300 rounded-md"
      ...
      case ProposalType.TREASURY_ALLOCATION:
        return (
          <div>
            <label className="block text-sm font-medium mb-1">Allocation Amount (SOL)</label>
            <input
              type="number"
              value={formData.fundingAmount}
              onChange={(e) => setFormData({ ...formData, fundingAmount: parseFloat(e.target.value) || 0 })}
      ...
      <div className="bg-white rounded-lg max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
        <div className="p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <select
                  ...
                  className="w-full px-3 py-2 border border-gray-300 rounded-md"
                >
                  <option value={ProposalType.PROJECT_FUNDING}>Project Funding</option>
                  <option value={ProposalType.MILESTONE_APPROVAL}>Milestone Approval</option>
                  <option value={ProposalType.PARAMETER_CHANGE}>Parameter Change</option>
                  <option value={ProposalType.TREASURY_ALLOCATION}>Treasury Allocation</option>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium mb-1">Discussion URL (Optional)</label>
                <input
                  type="url"
