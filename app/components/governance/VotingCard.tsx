...
export const VotingCard: React.FC<VotingCardProps> = ({
  ...
  const handleVote = async () => {
    ...
    try {
      ...
      logger.info('Vote cast successfully', { proposalId, option: selectedOption });
      onVoteCast();
      ...
      logger.error('Failed to cast vote', { error: (err as Error).message, proposalId, option: selectedOption });
        ...
        <CardContent>
          <div className="text-center space-y-3">
            ...
            <p className="text-sm text-gray-600">
              Voted on {new Date(voterInfo.lastVoteTime!).toLocaleDateString('en-US', {
                year: 'numeric',
                month: 'long',
                day: 'numeric',
                hour: '2-digit',
          ...
          >
            <div className="flex items-center gap-3">
              <CheckCircle className={`w-6 h-6 ${
                selectedOption === VoteOption.YES ? 'text-green-600' : 'text-gray-400'
              }`} />
          ...
          >
            <div className="flex items-center gap-3">
              <XCircle className={`w-6 h-6 ${
                selectedOption === VoteOption.NO ? 'text-red-600' : 'text-gray-400'
              }`} />
              <div className="text-left">
          ...
          >
            <div className="flex items-center gap-3">
              <AlertTriangle className={`w-6 h-6 ${
                selectedOption === VoteOption.ABSTAIN ? 'text-gray-600' : 'text-gray-400'
              }`} />
              <div className="text-left">
                <h3 className="font-medium text-gray-700">Abstain</h3>
                <p className="text-sm text-gray-600">Neither approve nor reject</p>
              </div>
            </div>
          </button>
        </div>
